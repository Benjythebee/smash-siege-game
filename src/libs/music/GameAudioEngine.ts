...
export class GameAudioEngine {
  ...
  constructor() {
    ...
    useUserSettings.subscribe((settings) => {
        this.trackOut.gain.value =
          settings.musicVolume < 0 ? 0.001 : settings.musicVolume > 1 ? 1 : settings.musicVolume
    })
  }
  ...
  getTracksForScene(state: 'menu' | 'playing' | 'default') {
    ...
    const mainTracksForScene =
      GameAudioEngine.mainTracks.get(state) ||
      GameAudioEngine.mainTracks.get('default')
  ...
  private _playNextTrack() {
    ...
    if (
      this.currentTrack &&
      this.trackPlayers.get(this.currentTrack) &&
      mainTracksForScene.includes(this.currentTrack)
    ) {
    ...
    let newTrack =
      mainTracksForScene &&
      mainTracksForScene[Math.floor(Math.random() * mainTracksForScene.length)]
